...
//        http.oauth2ResourceServer(t->t.opaqueToken(Customizer.withDefaults()));
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        ...
        http.addFilterAfter(createPolicyEnforceFilter(),
                BearerTokenAuthenticationFilter.class);

        http.sessionManagement(t->t.sessionCreationPolicy(SessionCreationPolicy.STATELESS));
        System.out.println("we are inside securityFilterChain");
        return http.build();
    }
    ...
    public DefaultMethodSecurityExpressionHandler msecurity(){
        System.out.println("we are inside msecurity");
    ...
    public JwtAuthenticationConverter con(){
        System.out.println("we are inside con");
